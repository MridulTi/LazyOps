...
def fix_misc_scripts() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    text = re.sub(
        r'AUTOMATION_ASSUME_ROLE_ARN="\$\{AUTOMATION_ASSUME_ROLE_ARN:-arn:aws:iam::\d+:role/[^"]+\}"',
        'AUTOMATION_ASSUME_ROLE_ARN="${AUTOMATION_ASSUME_ROLE_ARN:?Set AUTOMATION_ASSUME_ROLE_ARN}"',
        text,
    )
    write(path, text)
